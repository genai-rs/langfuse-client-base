...
**dataset_version** | Option<**chrono::DateTime<chrono::FixedOffset>**> | ISO 8601 timestamp (RFC 3339, Section 5.6) in UTC (e.g., \"2026-01-21T14:35:42Z\"). Specifies the dataset version to use for this experiment run.  If provided, the experiment will use dataset items as they existed at or before this timestamp. If not provided, uses the latest version of dataset items. | [optional]
**created_at** | Option<**chrono::DateTime<chrono::FixedOffset>**> | Optional timestamp to set the createdAt field of the dataset run item. If not provided or null, defaults to current timestamp. | [optional]
